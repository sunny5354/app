...
// Custom Imports
import { AgencyFormChildrenProps } from "../../types/Modal";
...
const AgencyFormModal: React.FC<AgencyFormChildrenProps> = ({
  modalVisible,
  handleModalVisible,
  selectedValue,
  handleSelectedValue,
  ...
  const handleSearch = (e: string) => {
    const searchedData = data.filter((val) => val.formname.toLowerCase().includes(e.toLowerCase()));
            ...
            </View>
            {
              dropDownData.map((i, k) => (
                <TouchableOpacity onPress={() => {
                  ...
                  {selectedValue === i._id && <FontAwesome name="dot-circle-o" size={24} color={colors.primaryGreen} />}
                </TouchableOpacity>
              ))
            }
          </ScrollView>
